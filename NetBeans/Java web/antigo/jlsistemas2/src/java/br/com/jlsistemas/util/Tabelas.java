...
public class Tabelas {
    ...
    public void TabelaSistema() {
     ...
     public void TabelaConsulta() {
        StringBuilder sql = new StringBuilder();
        sql.append("create table consulta(codigo serial not null PRIMARY KEY, paymentToken character varying(200) not null, chargeReference character varying(60) not null, chargeCode character varying(60) not null)");
        try {
            Connection conexao = ConexaoBD.conectar();
            PreparedStatement comando = conexao.prepareStatement(sql.toString());
            comando.execute();
            ...
            System.out.println("Erro ao criar tabela consulta" + ex.getMessage());
        }
    }
