...
public class TelaVeiculo extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel4.setText("Registrar veiculo");
        ...
        jLabel6.setText("Alterar veiculo");
        ...
        jLabel8.setText("Remover veiculo");
    ...
    private void jPanelNovo1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jPanelNovo1MouseClicked
        if (lista == null || lista.size() < 1 || jTablePadrao.getSelectedRow() < 0) {
            ...
            TelaVeiculoCadastro dialog = new TelaVeiculoCadastro(new javax.swing.JFrame(), true, lista.get(jTablePadrao.getSelectedRow()));
            dialog.setVisible(true);
            pesquisar();
            jTextFieldPesquisar.requestFocus();
        }
    ...
    private void jPanelNovo2MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jPanelNovo2MouseClicked
        if (lista == null || lista.size() < 1 || jTablePadrao.getSelectedRow() < 0) {
            ...
            int resposta = JOptionPane.showConfirmDialog(rootPane, "Deseja realmente remover " + lista.get(jTablePadrao.getSelectedRow()).getPlaca());
    ...
    private void jPanelNovo5MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jPanelNovo5MouseClicked
        if (lista == null || lista.size() < 1 || jTablePadrao.getSelectedRow() < 0) {
            JOptionPane.showMessageDialog(this, "Selecione um veiculo para ter acesso a seu desempenho", "Aviso", JOptionPane.WARNING_MESSAGE);
        }
    ...
    private void pesquisar() {
        if (jRadioButtonPlaca.isSelected()) {
            lista = dao.listar("veiculos.listarPlaca", "placa", jTextFieldPesquisar.getText());
        } else if (jRadioButtonMarca.isSelected()) {
            lista = dao.listar("veiculos.listarMarca", "marca", jTextFieldPesquisar.getText());
        } else if (jRadioButtonModelo.isSelected()) {
            lista = dao.listar("veiculos.listarModelo", "modelo", jTextFieldPesquisar.getText());
        } else {
    ...
    private void preencherTabela(List<Veiculo> ls) {
        String[] colunas = new String[]{"Codigo", "Placa", "Marca", "Modelo", "Ano"};
        ArrayList dados = new ArrayList();
        if (ls == null || ls.size() < 1) {
            dados.add(new Object[]{"Nenhum registro encontrado", "", "", "", ""});
        } else {
            for (Veiculo c : ls) {
                dados.add(new Object[]{c.getCodigo(), c.getPlaca(), c.getMarca(), c.getModelo(), c.getAno()});
            }
        }
