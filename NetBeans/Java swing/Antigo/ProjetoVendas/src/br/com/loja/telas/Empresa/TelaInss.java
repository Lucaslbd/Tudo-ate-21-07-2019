...
public class TelaInss extends javax.swing.JFrame {
    ...
    public TelaInss() {
        initComponents();
        preencherTabela("select * from inss order by valor_inicial");
    }
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        if (jTextFieldInicial.getText().isEmpty()) {
            ...
            if (flag == 1) {
                Inss inss = new Inss();
                inss.setValorInicial(Double.parseDouble(jTextFieldInicial.getText()));
                inss.setValorFinal(Double.parseDouble(jTextFieldFinal.getText()));
                inss.setPorcentagem(Double.parseDouble(jTextFieldPorcentagem.getText()));
                InssDao dao = new InssDao();
                dao.Salvar(inss);
            } else {
                Inss inss = new Inss();
                inss.setValorInicial(Double.parseDouble(jTextFieldInicial.getText()));
    ...
    private void preencherTabela(String sql) {
        ...
        String[] colunas = new String[]{"Codigo", "Valor inicial", "Valor final", "Porcentagem"};
        try {
            Connection conexao;
            ...
            while (resultado.next()) {
                dados.add(new Object[]{resultado.getInt("codigo"), resultado.getDouble("valor_inicial"), resultado.getDouble("valor_final"), resultado.getDouble("porcentagem")});
            }
            conexao.close();
