...
public class TelaGastoFuncionario extends javax.swing.JFrame {
    ...
    public TelaGastoFuncionario(Usuario u) {
        initComponents();
        usuario = u;
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Despesas com funcionários");
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        if (selecionado == 0) {
            JOptionPane.showMessageDialog(rootPane, "Selecione um pagamento para poder excluir");
        } else {
            int resposta;
            resposta = JOptionPane.showConfirmDialog(rootPane, "Deseja realmente excluir o pagamento?");
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        TelaGastoFuncionarioNovo dialog = new TelaGastoFuncionarioNovo(new javax.swing.JFrame(), true, usuario.getDeposito());
    ...
    private void Pesquisar() {
        ...
        if (jDateChooser1.getDate() == null || jDateChooser2.getDate() == null) {
            preencherTabela("select * from pagamento_funcionario inner join funcionario inner join empresa on empresa = codigo_empresa on funcionario = codigo_funcionario where empresa = " + usuario.getDeposito().getCodigo() + " and nome ilike'%" + jTextFieldPesquisa.getText() + "%' order by data");
        } else {
            preencherTabela("select * from pagamento_funcionario inner join funcionario inner join empresa on empresa = codigo_empresa on funcionario = codigo_funcionario where data between '" + jDateChooser1.getDate() + "' and '" + jDateChooser2.getDate() + "' and empresa = " + usuario.getDeposito().getCodigo() + " and nome ilike'%" + jTextFieldPesquisa.getText() + "%' order by data");
        }
        jList1.setVisible(false);
        pesquisou = true;
    }
    ...
    private void preencherTabela(String sql) {
        ...
        String[] colunas = new String[]{"Codigo", "Data", "Funcionário", "Cpf", "Descrição", "Valor"};
        try {
            Connection conexao;
            ...
            while (resultado.next()) {
                dados.add(new Object[]{resultado.getInt("codigo_pagamento"), util.ArrrumaData(resultado.getDate("data")), resultado.getString("nome"), resultado.getString("cpf"), resultado.getString("descricao"), util.ArrumaDouble(resultado.getDouble("valor"))});
            }
            conexao.close();
        ...
        jTablePadrao.getColumnModel().getColumn(2).setPreferredWidth(185);
    ...
    private void PreencherLista() {
        if (jTextFieldPesquisa.getText().isEmpty()) {
            ...
            sql.append("select * from funcionario inner join empresa on empresa = codigo_empresa where empresa =").append(usuario.getDeposito().getCodigo()).append(" and nome ilike'%").append(jTextFieldPesquisa.getText()).append("%' order by nome");
            try {
                Connection conexao;
                conexao = ConexaoBD.conectar();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(TelaGastoFuncionario.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new TelaGastoFuncionario().setVisible(true);
            }
        });
    }
