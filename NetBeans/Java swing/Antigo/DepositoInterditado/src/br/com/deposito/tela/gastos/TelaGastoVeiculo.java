...
public class TelaGastoVeiculo extends javax.swing.JFrame {
    ...
    public TelaGastoVeiculo(Usuario u) {
        initComponents();
        usuario = u;
    ...
    private void initComponents() {
        ...
        jComboBoxTipoPesquisa.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Motivo", "Descrição", "Placa", "Modelo" }));
        ...
        jLabel1.setText("Despesas com veiculos");
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        if (selecionado == 0) {
            ...
            JOptionPane.showMessageDialog(rootPane, "Ao remover um gasto também serão removidos seus respectivos registros");
            int resposta;
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        TelaGastoVeiculoNovo dialog = new TelaGastoVeiculoNovo(new javax.swing.JFrame(), true, usuario.getDeposito());
        dialog.setVisible(true);
    }//GEN-LAST:event_jButton1ActionPerformed

    private void Pesquisar() {
        String pesquisa = null;
        if (jComboBoxTipoPesquisa.getSelectedItem().equals("Motivo")) {
            pesquisa = "tipo";
        } else if (jComboBoxTipoPesquisa.getSelectedItem().equals("Descrição")) {
            pesquisa = "descricao";
        } else if (jComboBoxTipoPesquisa.getSelectedItem().equals("Placa")) {
            pesquisa = "placa";
        ...
        if (jDateChooser1.getDate() == null || jDateChooser2.getDate() == null) {
            preencherTabela("select * from gasto_veiculo inner join veiculo inner join empresa on empresa = codigo_empresa on veiculo = codigo_veiculo where empresa = " + usuario.getDeposito().getCodigo() + " and " + pesquisa + " ilike'%" + jTextFieldPesquisa.getText() + "%' order by data");
        } else {
            preencherTabela("select * from gasto_veiculo inner join veiculo inner join empresa on empresa = codigo_empresa on veiculo = codigo_veiculo where data between '" + jDateChooser1.getDate() + "' and '" + jDateChooser2.getDate() + "' and empresa = " + usuario.getDeposito().getCodigo() + " and " + pesquisa + " ilike'%" + jTextFieldPesquisa.getText() + "%' order by data");
        }
        jList1.setVisible(false);
        pesquisou = true;
    }
    ...
    private void preencherTabela(String sql) {
        ...
        String[] colunas = new String[]{"Codigo", "Data", "Motivo", "Descrição", "Placa", "Valor"};
        try {
            Connection conexao;
            ...
            while (resultado.next()) {
                dados.add(new Object[]{resultado.getInt("codigo_gastov"), util.ArrrumaData(resultado.getDate("data")), resultado.getString("tipo"), resultado.getString("descricao"), resultado.getString("placa"), util.ArrumaDouble(resultado.getDouble("valor"))});
            }
            conexao.close();
    ...
    private void PreencherLista() {
        ...
        if (jTextFieldPesquisa.getText().isEmpty()) {
            ...
            sql.append("select DISTINCT ").append(pesquisa).append("  from gasto_veiculo join veiculo inner join empresa on empresa = codigo_empresa on veiculo = codigo_veiculo where empresa = ").append(usuario.getDeposito().getCodigo()).append(" and ").append(pesquisa).append(" ilike'%").append(jTextFieldPesquisa.getText()).append("%' order by ").append(pesquisa).append("");
            try {
                Connection conexao;
                conexao = ConexaoBD.conectar();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(TelaGastoVeiculo.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new TelaGastoVeiculo().setVisible(true);
            }
        });
    }
